...
public class BTree {
    ...
    private void printArvore(No node, int nivel) {
        if (node != null) {
            printArvore(node.getvLig(node.getTL()), nivel + 1);
            for (int i = 0; i < nivel; i++) {
                System.out.print("    "); // Ajusta a indentação com espaços em branco
            }
            for (int i = 0; i < node.getTL(); i++) {
                System.out.print(node.getvInfo(i) + " "); // Imprime as chaves do nó
            }
            System.out.println(); // Nova linha para o próximo nível
            printArvore(node.getvLig(0), nivel + 1);
        }
    }
